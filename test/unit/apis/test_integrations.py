...
def _create_app_options(disable_scheduling: bool = False) -> MagicMock:
    ...
    app_options.cocktail_update_scheduling_disable_future_scheduling = disable_scheduling
    return app_options
...
class TestIntegrationsRouter:
    ...
    async def test_cocktail_updated_sync_routes_to_scheduling_handler_and_publishes_with_disabled_scheduling(self):
        """Test the inbound cocktail update route dispatches to the scheduling handler and publishes with disabled scheduling."""
        message_bus = MagicMock()
        message_bus.publish_event_async = AsyncMock()
        app_options = _create_app_options(True)
        handler = CocktailUpdatedSchedulingEventCommandHandler(message_bus=message_bus, app_options=app_options)
        mediator = _DispatchingMediator({CocktailUpdatedSchedulingEvent: handler})
